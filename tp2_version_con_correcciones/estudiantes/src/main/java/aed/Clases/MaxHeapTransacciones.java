...
public class MaxHeapTransacciones {
    private Heap<Transaccion> heap; // Cada handle va a ser (Transaccion, nodo) donde nodo significa la posicion
    ...
    public MaxHeapTransacciones(Transaccion[] transacciones) {
        ...
        Heap<Transaccion> _heap = new Heap(transacciones) ;
        heap = _heap;
    ...
    public void desencolar() {
        if (tamaño == 1 ){
            ...
            transaccionesOriginal[heap.devolverMax().nodo().posicion()] = null ;
        }
        heap.desencolar();
        tamaño -- ;  //O(1)
        // if (tamaño==1){   //O(1)
    ...
    public Transaccion transaccionMayorValor(){  //O(1)
        return heap.devolverMax().elemento();  //O(1)
    }
